...
# no need to clean chinese weibo data
def clean(sent):
    return sent
...
class GraphData():
    def __init__(self, args):
        ...
        self.pre_calculated_tail_list = self.pre_calculate_tail_list()

    def pre_calculate_tail_list(self):
        pre_calculated_tail_list = {}
        for from_index, row in enumerate(self.adj_matrix):
            row_adj = np.array(row)
            one_indices = np.where(row_adj == 1)[0]
            pre_calculated_tail_list[from_index+2] = [to_index+2 for to_index in one_indices]
            for to_index in one_indices:
                self.tail_to_head[to_index+2] = from_index+2
...
def _format_json_to_data(params):
    ...
    for dial in jobs:
        ...
        dial_example["src_triple"] = list()
        dial_example["tgt_triple"] = list()
        dial_example["entity"] = list()

        tgt = cleaned_sents

        dial_example["src_triple"].append(0)
        dial_example['tgt_triple'].append(0)
        dial_example["entity"].append(0)
        for word in src:
            if word in graph_processor.entity2idx.keys():
                head_idx = graph_processor.entity2idx[word]
                tail_list = graph_processor.pre_calculated_tail_list[head_idx]
                triples = []
                entities = []
                for tail_idx in tail_list:
                    triples.append([head_idx, tail_idx])
                    tail_word = graph_processor.idx2entity[tail_idx]
                    entities.append(tail_word)
                    # entities.append(bert.tokenizer.vocab[word[idx]])
                for idx in range(len(word)):
                    if len(triples) == 0:
                        dial_example['src_triple'].append(0)
                        dial_example['entity'].append(0)
                    else:
                        dial_example['src_triple'].append(triples)
                        entity_list = list()
                        for entity in entities:
                            if len(entity) > idx:
                                word_idx = idx
                            else:
                                word_idx = len(entity)-1
                            entity_list.append(bert.tokenizer.vocab[entity[word_idx]])
                        dial_example['entity'].append(entity_list)
            else:
        ...
        for word in tgt:
            if word in graph_processor.entity2idx.keys():
                tail_idx = graph_processor.entity2idx[word]
                if tail_idx in graph_processor.tail_to_head:
                    triple = [graph_processor.tail_to_head[tail_idx], tail_idx]
                else:
                    triple = [0, 0]
                for _ in range(len(word)):
                    dial_example['tgt_triple'].append(triple)
            else:
                for _ in range(len(word)):
                    dial_example['tgt_triple'].append([0, 0])
