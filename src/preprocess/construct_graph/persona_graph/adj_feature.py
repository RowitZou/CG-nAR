...
adj_list = []
keyword = ''
with open('src/preprocess/prepare_data/persona_graph.txt', 'r') as f_graph:
...
glove = open('resource/glove.6B.300d.txt', 'r')
word = list()
word_vector = list()
glove_dict = {}
line = glove.readline()
...
with open('src/preprocess/prepare_data/features.txt', 'wb') as features:
    pickle.dump(feature_matrix, features)

# with open('./prepare_data/features.txt', 'rb') as features:
#     features_m = pickle.load(features)
    # print(features_m[2])
    # print(len(features_m[2]))