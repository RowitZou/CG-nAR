...
from models.loss import CrossEntropyLossCompute
...
def build_trainer(args, device_id, model, optims, tokenizer):
    ...
    gen_loss = CrossEntropyLossCompute(model.generator, tokenizer.vocab, symbols).to(device)
...
class Trainer(object):
        ...
        while step <= train_steps:
            ...
            for i, batch in enumerate(train_iter):
                if self.n_gpu == 0 or (i % self.n_gpu == self.gpu_rank):
                    ...
                    concept_tokens += batch.tc_id.ne(self.abs_loss.padding_idx).sum().item()
                    src_tokens += batch.doc_src[:, 1:].ne(self.abs_loss.padding_idx).sum().item()
                    sents += batch.doc_src.size(0)
                    examples += batch.tgt.size(0)
                    accum += 1
                    if accum == self.grad_accum_count:
                        if self.n_gpu > 1:
                            concept_tokens = sum(distributed.all_gather_list(concept_tokens))
                        ...
                        normalization = (tgt_tokens, concept_tokens, src_tokens, sents, examples)
    ...
    def _gradient_calculation(self, true_batchs, normalization, total_stats,
        ...
        for batch in true_batchs:
            concept_out, _, dec_out, _ = self.model(batch)

            tgt_tokens, concept_tokens, src_tokens, sents, examples = normalization

            # concept loss
            if concept_out is not None:
                con_stats = self.abs_loss(batch.concept_tgt, concept_out,
                                          self.args.generator_shard_size,
                                          concept_tokens, retain_graph=True)
                total_stats.update(con_stats)
                report_stats.update(con_stats)

            # insertion TF loss
            abs_stats = self.abs_loss(batch.ins_tgt, dec_out,
                                      self.args.generator_shard_size,
