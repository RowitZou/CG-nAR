...
from others.utils import rouge_results_to_str, test_bleu, test_length, test_dist, test_f1
...
class Translator(object):
    ...
    def __init__(self,
        ...
        self.terminal_token = self.vocab['[unused3]']
    ...
    def from_batch_dev(self, doc_batch, tgt_data):
        ...
        for b in range(batch_size):
            ...
            pred_summ = pred_summ.replace('[unused1]', '').\
                replace('[unused2]', '').\
                replace('[PAD]', '').\
                replace('[UNK]', '').strip()
            pred_summ = ' '.join(pred_summ.split()).replace(" ##", "")

            gold_data = tgt_data[b]
            gold_data = gold_data.replace('[PAD]', '').\
                replace('[unused1]', '').\
                replace('[unused2]', '').\
                replace('[PAD]', '').\
                replace("[UNK]", '').strip()
            gold_data = ' '.join(gold_data.split()).replace(" ##", "")
    ...
    def validate(self, data_iter, step, attn_debug=False):
        ...
        with torch.no_grad():
            for batch in data_iter:
                ext_pred, ext_gold, output_data, tgt_data = self.translate_batch(batch)
        ...
        if (step != -1):
            ...
            pred_dist = test_dist(pred_path)
            file_rouge = FilesRouge()
            pred_rouges = file_rouge.get_scores(pred_path, gold_path, avg=True)
            f1, p, r = test_f1(ext_acc_num, ext_pred_num, ext_gold_num)
            self.logger.info('Concept Predict Score at step %d: \n>> P/R/F1: %.2f/%.2f/%.2f' %
            ...
            self.logger.info('Prediction Dist-1 at step %d: %.2f' %
                             (step, pred_dist[0] * 100))
            self.logger.info('Prediction Dist-2 at step %d: %.2f\n\n' %
                             (step, pred_dist[1] * 100))
            results = (pred_bleu*100,
                       pred_dist[0]*100,
                       pred_dist[1]*100)
        return results
    ...
    def translate(self,
        ...
        self.model.eval()
        output_path = self.args.result_path + '.%d.output.txt' % step
        ...
        if (step != -1):
            ...
            pred_rouges = file_rouge.get_scores(pred_path, gold_path, avg=True)
            f1, p, r = test_f1(ext_acc_num, ext_pred_num, ext_gold_num)
            self.logger.info('Ext Concept Score at step %d: \n>> P/R/F1: %.2f/%.2f/%.2f' %
                             (step, p*100, r*100, f1*100))
            self.logger.info('Gold Length at step %d: %.2f\n' %
                             (step, test_length(gold_path, gold_path, ratio=False)))
            self.logger.info('Prediction Length ratio at step %d: %.2f' %
    ...
    def translate_batch(self, batch):
        ...
        """
        _, concept_data, _, text_data = self.model(batch)
        tgt_txt = batch.tgt_txt
        tgt_c = list(map(lambda x: [int(n) for n in x
                                    if n.item() not in [0, 1]],
                         batch.tc_id))
        return concept_data, tgt_c, text_data, tgt_txt
