...
class Model(nn.Module):
        ...
        if args.encoder == 'bert':
            ...
            if(args.max_pos > 512):
                ...
                my_pos_embeddings.weight.data[:512] = \
                    self.encoder.model.embeddings.position_embeddings.weight.data
                my_pos_embeddings.weight.data[512:] = \
                    self.encoder.model.embeddings.position_embeddings.weight.data[-1][None, :].repeat(args.max_pos-512, 1)
    ...
    def _fast_translate_batch(self, batch, memory_bank, max_length, memory_mask=None,
        ...
        if self.args.decoder == "rnn":
            dec_states = self.decoder.init_decoder_state(batch.doc_src, memory_bank, hidden_state)
        else:
            dec_states = self.decoder.init_decoder_state(batch.doc_src, memory_bank, with_cache=True)
    ...
    def forward(self, batch):
        ...
        if self.args.encoder == "bert":
            ...
            top_vec = self.encoder(src_emb, ~mask_src)
        elif self.args.encoder == "rnn":
            top_vec, hid = self.encoder(src, mask_src)

        # hierarchical encoding
        sent_list = torch.split(top_vec[:, 0, :], ex_segs)
        ...
        if self.training:
            if self.args.decoder == "rnn":
                ...
                dec_state = self.decoder.init_decoder_state(src, hier_hid)
                decode_output, _, attn = self.decoder(tgt[:, :-1], hier_hid, dec_state, memory_masks=sent_mask,
            ...
            if self.args.decoder == "rnn":
                ...
                summary = self._fast_translate_batch(batch, hier_hid, self.max_length, memory_mask=sent_mask,
